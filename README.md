...
workspace is `v0.3.4`. It is usable for local coding workflows, but still pre-1.0: APIs, command
...
`v0.3.4` is suitable for local developer use where the operator reviews tool actions and keeps
...
- Session scheduling: `delay` (schedule a one-shot delayed follow-up message; session-scoped, dies on process exit; Execute/Ask permission)
...
is the source of truth for the published `v0.3.4` release announcement and downloads.
