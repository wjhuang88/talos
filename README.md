...
workspace is `v0.6.1`. It is usable for local coding workflows, but still pre-1.0: APIs, command
...
`v0.6.1` is suitable for local developer use where the operator reviews tool actions and keeps
...
Replace `v0.6.1` with a tag listed on
...
is the source of truth for the published `v0.6.1` release announcement and downloads.
