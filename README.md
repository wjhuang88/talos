...
| Area | State | Notes |
|------|-------|-------|
| Runtime | Active | Event-driven TUI: `talos-conversation` crate separates business logic from UI rendering. Two-loop design (Agent → ConversationEngine → UI) via typed async channels. Codex-style `insert_history`, stream-based content delivery, single-directional flow. Styled scrollback with Nord bg for user messages, 3-column line padding, multiline paste blocks, single-row preview with Markdown block classification for tables/code fences/lists/quotes, animated braille spinner, native cursor sync. 110 TUI+conversation tests pass (57 TUI + 53 conversation). |
