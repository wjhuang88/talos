...
workspace is `v0.7.0`. It is usable for local coding workflows, but still pre-1.0: APIs, command
...
`v0.7.0` is suitable for local developer use where the operator reviews tool actions and keeps
...
Replace `v0.7.0` with a tag listed on
...
- Process execution: `exec` (argv-only single process, no shell parsing), plus the permission-gated platform shell escape hatch: `bash` through `sh -c` on Unix and `powershell` through `powershell.exe -NoProfile -NonInteractive` on Windows
...
is the source of truth for the published `v0.7.0` release announcement and downloads.
