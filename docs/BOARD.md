...
*[No items in Review — all resolved 2026-06-29]*
...
| Plugin Encapsulation Architecture (owner draft) | Draft awaiting decision | [proposal](proposals/plugin-encapsulation-format.md) | Four-entity model (skill/mcp/hook atomic + plugin packaging). Owner draft 2026-06-30. Carrier set settled: WASM first-class, Lua optional, **dylib rejected** (2026-06-30). Blocks PLUGIN-001, CMD-002, HOOK-001, TOOL-008 Phase 3. Remaining gate: runtime boundary ADR (WASM engine/sandbox/host-call surface) before implementation. |
| PLUGIN-001 Plugin Encapsulation System | Blocked | [PLUGIN-001](backlog/active/PLUGIN-001-wasm-runtime-plugins.md) | Repositioned from WASM-only to full four-entity plugin model (carriers: WASM first-class, Lua optional, dylib rejected 2026-06-30). Blocked on the proposal's ADRs (WASM runtime/sandbox boundary, provenance extension, atomic component model, command taxonomy). |
| CMD-002 Command Taxonomy Realignment | Blocked | [CMD-002](backlog/active/CMD-002-command-taxonomy-realignment.md) | `/plugins` is misoccupied by MCP status. Blocked on plugin entity existing + proposal ADR #4. |
| HOOK-001 Config-Introduced Hooks | Blocked | [HOOK-001](backlog/active/HOOK-001-config-introduced-hooks.md) | Hook must become a config-introduced peer of skill/mcp. Blocked on proposal ADR #3 + carrier-strategy ADR. |
...
| REL-002 v1.0 Self-Bootstrap Release Gate | Planned | [REL-002](backlog/active/REL-002-v1-self-bootstrap-release-gate.md) | `v1.0.0` cannot be claimed until Talos performs 100% self-bootstrap development as the primary runtime, with recorded evidence. |
...
| WEB-005 Browser Session Continuity | Planned | [WEB-005](backlog/active/WEB-005-browser-session-continuity-research.md) | Implement authenticated browser-page context as a conditional `fetch_url` backend with safe page records and separate browser-page read permissions; no standalone browser tool group or automation before ADR. |
