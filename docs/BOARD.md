...
| ARCH-031 Crate Publication Boundary | In Progress (high-risk crates deferred) | [ARCH-031](backlog/active/ARCH-031-crate-publication-boundary.md) / [matrix](reference/CRATE-PUBLICATION-MATRIX.md) | Published 0.2.0: `talos-core`, `talos-skill`, `talos-config`, `talos-permission`, `talos-session`, `talos-plugin`, `talos-memory`, `talos-exploration`, `talos-provider`, `talos-conversation`, and `talos-rpc`. Defer sandbox/tools/agent/runtime/MCP/CLI/TUI until safety/API gates are explicit. |
