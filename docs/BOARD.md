...
|---|---|---|---|
| I023 TUI State Model | Review | [I023 TUI State Model](iterations/I023-tui-state-model.md) | Event-driven architecture: `talos-conversation` + `talos-tui` separation; Codex-style `insert_history` with styled scrollback; 3-char ASCII line padding; user messages with Nord bg color + top/bottom padding; single-row preview with Markdown block classifier for tables/code fences/lists/quotes; animated braille spinner; native cursor sync; 110 tests pass (57 TUI + 53 conversation) |
