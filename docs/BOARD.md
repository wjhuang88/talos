...
| ARCH-031 Crate Publication Boundary | In Progress (boundary docs next) | [ARCH-031](backlog/active/ARCH-031-crate-publication-boundary.md) / [matrix](reference/CRATE-PUBLICATION-MATRIX.md) | Published 0.2.0: `talos-core`, `talos-skill`, `talos-config`, `talos-permission`, `talos-session`, `talos-plugin`, `talos-memory`, and `talos-exploration`. `talos-provider` and `talos-conversation` dry-run pass but need support boundary docs before real publish. |
