...
| ARCH-031 Crate Publication Boundary | In Progress (classification enforced) | [ARCH-031](backlog/active/ARCH-031-crate-publication-boundary.md) / [matrix](reference/CRATE-PUBLICATION-MATRIX.md) | 11 crates published at 0.2.0. `talos-cli`, `talos-tui`, and `talos-evolution` are product-only with `publish = false`; sandbox/tools/agent/runtime/MCP require explicit safety/API gates before publish. |
