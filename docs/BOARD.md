...
| I057 Acceptance Remediation And Release Gate | Review | [I057](iterations/I057-acceptance-remediation-and-release-gate.md) | All 5 stories delivered. User approved completing a release on 2026-06-27; v0.2.0 can be tagged after validation. |
...
| Two-Month Architecture Optimization | Complete | [task](tasks/2026-06-27-two-month-architecture-optimization-plan.md) | M0-M11 complete; remaining production roots are explicitly registered under ARCH-030. Future architecture work should activate one residual root at a time. |
...
| I071 Agent Configuration Decomposition | Complete | [I071](iterations/I071-agent-configuration-decomposition.md) | Agent constructors/setters now live in `configuration.rs`; `lib.rs` 914→655 lines; duplicate prompt-builder mutation centralized; workspace/governance/diff gates pass. |
| I072 Conversation Command Registry Decomposition | Complete | [I072](iterations/I072-conversation-command-registry-decomposition.md) | Command registry metadata/completion now lives in `command_registry.rs`; `engine.rs` 960→739 lines; workspace/governance/diff gates pass. |
| I073 OpenAI Request Assembly Decomposition | Complete | [I073](iterations/I073-openai-request-assembly-decomposition.md) | OpenAI request DTOs/body assembly/redaction now live in `openai_request.rs`; `openai.rs` 1001→848 lines; workspace/governance/diff gates pass. |
| I074 Exploration Types Decomposition | Complete | [I074](iterations/I074-exploration-types-decomposition.md) | Exploration domain entities now live in `types.rs`; `lib.rs` 1070→958 lines; workspace/governance/diff gates pass. |
...
| I056 Two-Month Closeout And v0.2.0 Readiness | Review | [I056](iterations/I056-two-month-closeout-and-v020-readiness.md) | All gates green, I019/I020/DATA-001 synced, README updated, release decision package ready. User approved v0.2.0 release execution on 2026-06-27. |
...
| WEB-002 GitHub Pages Product Site And Custom Domain | Review (site built, awaiting Pages publish) | [WEB-002](backlog/active/WEB-002-github-pages-product-site.md) / [handoff](tasks/2026-06-29-delegable-product-site-docs-two-month-plan.md) | Static `site/` and user-doc sync are complete. `sh scripts/validate_public_site.sh` and `sh scripts/validate_project_governance.sh .` both exit 0. Maintainer enables Pages (Settings → Pages → Deploy from a branch, folder `/site`), optionally enables a custom domain via `site/CNAME`, and runs both validators on the published site. No DNS, tag, release, or workflow change was made by the docs executor. |
