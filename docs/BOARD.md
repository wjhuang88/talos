...
| ARCH-031 Crate Publication Boundary | Blocked on crates.io account email | [ARCH-031](backlog/active/ARCH-031-crate-publication-boundary.md) / [matrix](reference/CRATE-PUBLICATION-MATRIX.md) | `talos-core` and `talos-skill` dry-run pass; real `talos-core` publish attempted from `30c9abc`, but crates.io rejected upload until publisher email is verified. |
