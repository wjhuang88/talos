...
| WEB-002 GitHub Pages Product Site And Custom Domain | Review (CNAME created, awaiting Pages enable + DNS) | [WEB-002](backlog/active/WEB-002-github-pages-product-site.md) / [handoff](tasks/2026-06-29-delegable-product-site-docs-two-month-plan.md) | Static `site/` and user-doc sync complete. `site/CNAME` set to `talos.hwj.zone`. Validators pass. **Remaining maintainer actions**: (1) Add DNS CNAME `talos` → `wjhuang88.github.io`, (2) Enable Pages (Settings → Pages → Deploy from a branch, folder `/site`), (3) Enter custom domain `talos.hwj.zone` in Pages settings. No tag, release, or workflow change. |
