...
| WEB-002 GitHub Pages Product Site And Custom Domain | Review (CNAME + workflow in place, awaiting Pages enable + DNS) | [WEB-002](backlog/active/WEB-002-github-pages-product-site.md) / [handoff](tasks/2026-06-29-delegable-product-site-docs-two-month-plan.md) | Static `site/` complete. `site/CNAME` set to `talos.hwj.zone`. `.github/workflows/pages.yml` committed ("Deploy from a branch" does not support `/site`). Validators pass. **Remaining maintainer actions**: (1) Add DNS CNAME `talos` → `wjhuang88.github.io`, (2) Settings → Pages → Source: "GitHub Actions", (3) Enter custom domain `talos.hwj.zone`. No tag or release change. |
...
| I040 Session Foundation & Tool Refinement | Complete | [I040](iterations/I040-session-foundation-tool-refinement.md) | SESSION-001-A + http_request content detection + save_url + fetch_url |
...
| TOOL-014 Conditional Tool Backends | Partial (continuation core complete) | [TOOL-014](backlog/active/TOOL-014-conditional-tool-backends.md) | Backend/schema disclosure and `ToolResult` continuation handling landed with execution guard and tests; `fetch_url` backend migration remains. |
