...
| Plugin Encapsulation Architecture | Accepted baseline | [proposal](proposals/plugin-encapsulation-format.md) / [ADR-027](decisions/027-plugin-runtime-boundary.md) / [ADR-028](decisions/028-plugin-tool-provenance-extension.md) / [ADR-029](decisions/029-extensibility-atomic-component-model.md) / [ADR-030](decisions/030-extensibility-command-taxonomy.md) | Architecture decisions accepted. Implementation gates: focused `wasmtime` dependency/security review before runtime code; no dylib, Lua, marketplace, remote install, or auto-discovery in first slice. |
| PLUGIN-001 Plugin Encapsulation System | Planned | [PLUGIN-001](backlog/active/PLUGIN-001-wasm-runtime-plugins.md) | Start with local explicit WASM plugin MVP after ADR-027 dependency/security review: manifest parser, plugin provenance, permission-gated read-only tool, trap/timeout/error tests. |
| CMD-002 Command Taxonomy Realignment | Planned | [CMD-002](backlog/active/CMD-002-command-taxonomy-realignment.md) | ADR-030 accepted. First slice moves MCP status to `/mcp`; `/plugins` shows a transition notice, not an alias, until plugin packages ship. |
