...
| ARCH-031 Crate Publication Boundary | In Progress (first-wave crates published) | [ARCH-031](backlog/active/ARCH-031-crate-publication-boundary.md) / [matrix](reference/CRATE-PUBLICATION-MATRIX.md) | `talos-core`, `talos-skill`, `talos-config`, `talos-permission`, and `talos-session` 0.2.0 are published. `talos-runtime` waits for implementation dependency publication/decoupling. |
