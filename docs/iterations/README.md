...
| I195 | Dashboard Read-Only Visual Shell | Active / Claimed | Claim PR #212 merged as `f123e534`; activation PR #288 starts the bounded read-only Dashboard slice from `main@4635ef2b`. #233 remains Draft provenance and must be rebuilt from the activation merge or later current `main` before review. |
...
| I205 | PR Workflow Throughput Simplification | Active / Claimed / PR #284 | Claim merge `fd1eaad9`; evidence-only governance Spike. I195 runs in parallel only under explicit 2026-08-18 maintainer non-overlap authorization; neither iteration imports the other's authority. Every executable rule change remains excluded and needs a new bounded claim. |
