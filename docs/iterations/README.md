...
| I085 | Model Catalog Modernization — talos-models, /model, /connect | **Paused (2026-07-04; MC107 manual TUI residual)** | Stage 1 complete. Stage 2 core gaps and post-review fixes landed, then the runtime `catalog.db` path was superseded by the 2026-07-05 maintainer decision: `/connect` and `/model` use packaged offline `models.toml`, while refresh is build-time via `BUILD_MODELS=1`. Remaining residual: real terminal `/connect` walkthrough. See `I085-model-catalog-modernization.md`. |
...
| I106 | Self-Bootstrap Control Plane | Active | Month 1 of the 2026-07-08 Talos-primary self-bootstrap plan. SBT100 activated 2026-07-09. Establish execution contract, evidence schema, runtime smoke harness, and governance rehearsal before claiming any REL-002 progress. See `I106-self-bootstrap-control-plane.md`. |
| I107 | Talos-Primary Feature Polish | Planned | Month 2 of the 2026-07-08 Talos-primary self-bootstrap plan. Talos must first address the issue-audit corrective queue, starting with #18 request-dispatch timeout, then proceed to lower-risk feature/polish work if capacity remains. See `I107-talos-primary-feature-polish.md`. |
...
| I106 | Active | Month-1 Talos-primary self-bootstrap control plane. SBT100 activated 2026-07-09. Preserve evidence/disqualification rules; runtime is external (glm-5.2), so session is non-qualifying unless Talos binary becomes primary. |
| I107 | Planned | Month-2 Talos-primary corrective feature/polish session. Activate after I106 closeout; select #18 request-dispatch timeout before TOOL-020 or other polish. |
