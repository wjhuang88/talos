...
| I085 | Model Catalog Modernization — talos-models, /model, /connect | **Active (2026-07-03; Stage 2 gap-fix closeout)** | Activated after `v0.2.2` closeout. Stage 1 complete: talos-models SQLite crate, correct models.dev parsing, gated refresh, resolver precedence. Stage 2: catalog-backed `/connect` with optional `base_url`, and group-aware search filtering for `/model`+`/connect`, both landed; MC107 (README depth, manual TUI verification) remains open. See `I085-model-catalog-modernization.md`. |
...
| I085 | Active | Activated 2026-07-03 after `v0.2.2` release closeout. Stage 1 (MC100-MC103) complete; Stage 2 `/connect`+`/model` core acceptance (catalog wiring, base_url, group-aware filtering) landed. MC107 residual: README onboarding depth, manual interactive TUI verification. |
