...
| I195 | Dashboard Read-Only Visual Shell | Planned / proposed Claimed | Governance PR #212 proposes only WEB-001-A's user-facing IA and read-only visual shell. The claim is ineffective before the finalized exact head reaches `main`; no implementation branch/worktree may exist before then. I193 is now effective target-branch truth; I194 Desktop remains an independent unmerged proposal and is not imported as target-branch truth. |
