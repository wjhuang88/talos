...
| I023 | TUI State Model | Planned | Unified `ChatMessage`/`Tip`/`TuiStateEvent` model replaces flat `ChatLine`/`status_message` fields. Removes message lifecycle bugs by construction, opens event-bus hook. Depends on I022 (landed). See `I023-tui-state-model.md`; `docs/backlog/active/TUI-004-state-model.md`. |
...
| R14: I023 TUI State Model | Next iteration (user-prioritized) | `I023-tui-state-model.md` | Unified `ChatMessage`/`Tip`/`TuiStateEvent` model replaces flat state; event-bus hook opens for future global state observation. |
