...
| I022 | TUI Inline-by-Default | **Planned** (2026-06-07) | Adopt Codex TUI's inline-by-default architecture: viewport at cursor y, finalized turns push-appended to scrollback via `insert_history_lines` (Codex `codex-rs/tui/src/insert_history.rs` with `SetScrollRegion(1..viewport.top())`); `EnterAlternateScreen`/`LeaveAlternateScreen` removed from the default code path. This is the architectural foundation for TUI-002 sub-slice A. New `tui/` subdir (custom_terminal, insert_history, event_stream, frame_requester, frame_rate_limiter, job_control, keyboard_modes) + `history_cell/` base cells. SIGTSTP job control per [ADR-018](../decisions/018-tui-job-control-unsafe.md). Absorbs the original TUI-003. Sub-slices B-E (bottom_pane/composer, slash_command framework, keymap system, tui/ refinements) remain blocked behind I015-I017. See `I022-tui-inline-default.md`; `docs/proposals/tui-codex-overhaul.md`; `docs/reference/codex-tui-architecture.md`; ADR-018. |
...
| R13: I022 TUI Inline-by-Default | Next TUI architecture slice (after I014, ahead of I015-I017 foundations for sub-slice A only) | `I022-tui-inline-default.md` | `talos-tui` adopts Codex-style inline-by-default TUI; `EnterAlternateScreen`/`LeaveAlternateScreen` removed; new `tui/` and `history_cell/` subdirs per [ADR-018](../decisions/018-tui-job-control-unsafe.md); I014 functionality (provenance, `/plugins`, `/copy`, `/export`) preserved; I008 hook ordering preserved; 652/652 tests pass; runtime evidence recorded |
