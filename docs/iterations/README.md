...
| I202 | Dashboard Availability In The Logo Prefix | Planned / proposed claim pending | TUI-037 / Issue #104 selected before the mainline long task. Governance-only claim must reach `main` before implementation; current complete behavior is a token-free plain-text Logo row with no OSC 8 and no token logging. |
