...
| I023 | TUI State Model | **Review** (2026-06-09) | S1-S6 implemented: `ChatMessage`/`Tip`/`TuiStateEvent` replaces `ChatLine`/`status_message`; `ViewportLayout` struct with auto-computed `HEIGHT`; `InlineTerminal::new(viewport_height)` parameterized; bg-color input area with `❯` prompt; `TipKind`-colored tips; Nord-styled status bar; 131 TUI tests pass; `ChatLine` removed. Awaiting final runtime verification. See `I023-tui-state-model.md`. |
...
| R14: I023 TUI State Model | ✅ Done (2026-06-09) | `I023-tui-state-model.md` | `ChatMessage`/`Tip`/`TuiStateEvent` unified model; `ViewportLayout` struct; 131 tests; awaiting final runtime verification |
