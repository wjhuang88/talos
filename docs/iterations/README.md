...
| I023 | TUI State Model | **Review** (2026-06-11) | Event-driven architecture: `talos-conversation` + `talos-tui` separation. Codex-style single-row history insertion with styled scrollback, 3-column prefix padding, multiline user messages with Nord bg color + top/bottom padding, one-row preview with Markdown block classifier, conservative styled Markdown rendering, animated braille spinner, native cursor sync. 113 focused tests pass (60 TUI + 53 conversation). See `I023-tui-state-model.md`. |
...
| R14: I023 TUI State Model | Review (2026-06-11) | `I023-tui-state-model.md` | Event-driven architecture with `talos-conversation` crate; Codex-style single-row history insertion; Markdown block classifier and styled renderer landed; 113 focused tests; one-week closure plan covers cancellation, non-lossy state delivery, engine-owned mutation, runtime verification |
