...
| I026 | Approval UX + Git Tools + Prompt Optimization | **Review** | All 7 stories implemented: approval ordering and inline result display, read/write Git tools, dynamic prompt templates, Anthropic cache-control emission, tree tool, and active documentation validation. Workspace clippy/tests/governance validation recorded in `I026-approval-ux-doc-validation.md`. |
...
| R16: Two-Week Handoff | Active | `../roadmap/TWO-WEEK-HANDOFF-PLAN.md`; `I024-conversation-context.md`; `../backlog/active/TUI-005-logo-splash.md` | TUI-005 in-scope splash delivered and corrected: branded inline `TALOS` block wordmark with Nord Frost gradient, `⬡ The watchman never sleeps` tagline, narrow-width fallback, fresh-line startup boundary, and preserved blank line before logo; README synced, workspace verification clean (82 TUI tests). Phase 3-4 viewport overlay deferred per Scope Boundary. |
