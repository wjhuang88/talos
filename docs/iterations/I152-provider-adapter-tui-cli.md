...
| 2026-07-22 | Filename rendering repair | Real image submission succeeded, but the Markdown-rendered system attachment notice swallowed underscores in the basename. The safe history summary was already correct. The bridge now inserts a display-only zero-width separator after underscores in that system-only basename display, preventing Markdown interpretation without showing escape characters; image path, digest, and provider payload behavior are unchanged. |
