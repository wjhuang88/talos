...
| 2026-07-22 | Filename rendering repair | Real image submission succeeded, but the Markdown-rendered system attachment notice swallowed underscores in the basename. The safe history summary was already correct. The bridge now escapes Markdown metacharacters in that system-only basename display; image path, digest, and provider payload behavior are unchanged. |
