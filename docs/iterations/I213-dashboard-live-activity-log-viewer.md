...
| Handoff / Release Condition | Preserve the controlled parallel exceptions recorded in the appended checkpoints. Before implementation merge, obtain I213 exact-head CI and independent security review, refresh open-PR/file-overlap inventory, and repeat merge-time CAS. |
...
The explicit parallel authorizations are scheduling variances only. They do not change any owner
boundary, expire when either named pair is no longer Active and must not be reused for another
iteration.
...
Maintainer coordination comment `5386904546` supersedes the earlier serial-pause note
`5386854800` and authorizes only the exact I213/I222-B pair to run concurrently. I213 retains its
existing 17-file PR #372 Dashboard/live-activity inventory. I222-B owns core/agent/tools/runtime and
must not edit `crates/talos-cli/**`, `crates/talos-dashboard/**`, README files or I213/WEB product
authority. Shared Board/backlog/index/manifest updates use union semantics. Before stable push and
merge, both lanes compare exact inventories; same production-file or authority overlap pauses only
the overlapping work. No I213 CI/review evidence carries to I222, and C/D cannot reuse this waiver.
