...
- Catalog-in-`/connect` evidence: `build_connect_picker_data_uses_catalog_provider_metadata`,
  ...
  `open_catalog_snapshot_missing_file_returns_none`,
...
- Stage 2 is unblocked and its core acceptance gaps are closed: `/connect` reads
  ...
  filter" search. MC107 residuals below remain.
- MC107 residual: manual TUI runtime verification of `/connect` end-to-end
  ...
  session.
- MC107 residual: this README does not yet document `/connect` in its own
  paragraph beyond the slash-command table entry added in this pass; a fuller
  onboarding walkthrough (screenshot or transcript) is deferred.
- `catalog.db` is never auto-populated in Stage 1/2 (by design — no startup
  network access). Until an explicit `--fetch-models`/seed workflow exists, a
  fresh install's `/connect` will use the `builtin_models()` fallback path,
  which does not carry `api_base_url`/`doc_url` metadata. This is expected
  under the current non-goals ("No auto-fetch at startup") and is not a defect
  of this pass; it becomes relevant once a seed/fetch command is implemented.
