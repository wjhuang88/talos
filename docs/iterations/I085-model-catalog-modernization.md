...
- Stage 2 is no longer gated, but it is not complete. Commit `d7e37df`
  delivered the primary `/connect` command path and provider-grouped `/model`
  display, while leaving these I085 acceptance gaps:
  - MC104: `/connect` still builds from `builtin_models()` rather than
    `catalog.db`, so full models.dev provider metadata and 100+ provider
    discovery are not yet accepted.
  - MC104: optional custom endpoint/base_url input and merge behavior are not
    implemented.
  - MC106: grouped search filtering is incomplete; `filtered_indices` or an
    equivalent original-index-preserving helper must hide empty provider groups
    while keeping headers non-navigable.
  - MC107: README `/connect` documentation, owner-doc checkbox sync, manual TUI
    verification, and final closeout remain pending.
