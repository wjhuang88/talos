...
- [~] #I004-S5: Process hardening basics — ⚠️ **built but unwired** (see note below; remediation #ARCH-S3)
...
- [~] Process hardening: env sanitization, resource limits, core dump prevention — ⚠️ **implemented + unit-tested but never invoked by any runtime path** (see "Baseline Correction" below)
...
## Baseline Correction (2026-06, architecture review)

> Recorded per `docs/iterations/README.md` rule: append corrections, do not silently overwrite a
> published baseline.

**#I004-S5 (Process hardening) was marked complete prematurely — it is a *security illusion*.**

- `ProcessHardening` (`crates/talos-sandbox/src/hardening.rs`) is fully implemented and unit-tested
  (12 tests), **but `apply()` is never called by any production execution path.** The env
  scrubbing and `setrlimit` resource limits therefore have **zero runtime effect** on executed
  commands. Unit tests passed, so the gap was invisible at merge time.
- Additionally, the four production `unsafe` blocks in `hardening.rs` shipped **without the ADR**
  required by AGENTS.md Hard Constraint #2 (a compliance gap acknowledged in `EVOLUTION.md`).

**Remediation (tracked, do not re-open this iteration):**
- [ADR-007](../decisions/007-process-hardening-unsafe.md) — records & justifies the `unsafe` sites (closes the compliance gap).
- **#ARCH-S1** (backlog) — annotate the `unsafe` blocks to reference ADR-007.
- **#ARCH-S3** (backlog) — wire `ProcessHardening` into the **child** bash subprocess via `pre_exec`; this is what genuinely satisfies the original #I004-S5 acceptance criteria.

Until #ARCH-S3 lands, treat #I004-S5 as **incomplete-by-runtime-evidence** despite passing unit tests.
