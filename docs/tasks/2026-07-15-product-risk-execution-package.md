...
| 2026-07-15 | P110 complete | `6deae69` + supplement | I130 Review | In-memory Up/Down history with draft restoration, dedup, boundaries. 9 state tests + 5 entry-point tests calling actual handle_input_event with Event::Key. No persistence, transcript, or format change. Full validation green (312 TUI tests). | `crates/talos-tui/src/state.rs`, `crates/talos-tui/src/app.rs`, `crates/talos-tui/src/state_tests.rs`, `crates/talos-tui/src/app/app_tests.rs` | No deps/API/format/persistence change. | Entry-point handle_input_event tests added per architecture review v2; await re-review before P120. |
