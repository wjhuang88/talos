...
| LT024 | I117 closeout | Security-reviewed limitation and residual report | LT022, LT023 | full preflight and permission security review | I117 remains Partial; PERM-005 owns residual | Complete: independent conservative-slice review, full preflight, governance, smoke, permission and production-input tests passed |
...
| LT032 | Installer validation | Site entrypoints checked against canonical scripts, checksums, assets and offline behavior | LT024 | validator/dry-run fixtures pass | Keep GitHub/raw scripts canonical; do not change default URL | Complete: real `install.sh` driven through a PATH-injected local transport; install/latest/checksum-mismatch/offline cases pass without network |
...
| 2026-07-12 | LT020-LT024 partial (I117 — evidence diagnostic-only) | ADR-040 written; `access_evidence.rs` in talos-permission; trust status/revoke in permissions.rs; 107 permission tests; auto-Allow removed (evidence ≠ authority) | `release_preflight.sh` passed; governance 0 warnings; diff clean; smoke 13/13 | Evidence API not wired into bash/exec execution pipeline; formal security sign-off not recorded; LT020/LT022/LT024 marked Partial | Proceed to LT030-LT034 (I118 bounded local productization) after recording I117 as Partial |
...
| 2026-07-12 | I117/I118 corrective review | Exec single/steps/pipes diagnostic coverage and production-input tests added; installer fixture now drives the real POSIX installer through a no-network local transport; independent security review recorded | Permission/agent targeted tests and installer fixture 4/4 passed | Evidence remains diagnostic-only; PowerShell behavior remains statically validated on this non-Windows host; LT002 remains manual | Run full preflight in a socket-capable environment, then complete LT002 in a real terminal |
| 2026-07-12 | Corrective validation | Canonical preflight rerun outside the restricted socket sandbox; dashboard loopback tests and the full workspace passed. Two allocated-PTY LT002 attempts reached `ESC[6n` but could not emulate a cursor-position response. | `release_preflight.sh`: passed; clippy targeted crates: passed; permission 107, agent 215, TUI 257; installer fixture 4/4; smoke 13/13; governance 0 warnings | Only LT002 remains; failure is recorded rather than treated as TUI acceptance | In a real terminal, run the disposable-HOME LT002 walkthrough at lines 59-77, append observed `/connect` and `/model` results here and in I085, then mark LT002 and this task Complete |
| 2026-07-12 | LT002 complete | Disposable HOME in a detached `screen` terminal emulator; no credential entered or saved | `/connect`: 151 providers rendered, `openai` filter and credential view passed, Esc cancelled; `/model`: DeepSeek group rendered; normal exit, zero turns | None | Long task complete; retain REL-002 NO-GO and no-release posture |
...
- Future permission broadening: PERM-005/ADR-040 re-review trigger; the accepted slice remains
  diagnostic-only and grants no command auto-Allow authority.
