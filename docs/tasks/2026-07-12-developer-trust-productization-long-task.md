...
| LT023 | Trust status and revoke | Explicit read-only status and revocation UX for Git workspace trust | LT021 | persistence, redaction, non-Git strictness tests | Ship diagnostics/revoke without command trust | Planned |
| LT024 | I117 closeout | Security-reviewed limitation and residual report | LT022, LT023 | full preflight and permission security review | I117 remains Partial; PERM-005 owns residual | Planned |
| LT030 | Local extension diagnostics | Explicit local read-only plugin/command/hook discovery with provenance/collision diagnostics | LT024 | fixtures and CLI smoke; no executable carrier | Diagnostics only | Planned |
| LT031 | Bounded document extraction | Text/HTML/JSON/CSV/Markdown extraction with size/type/permission guards | LT024 | failure and handoff tests through binary | Preserve existing formats; defer heavy/native parser | Planned |
| LT032 | Installer validation | Site entrypoints checked against canonical scripts, checksums, assets and offline behavior | LT024 | validator/dry-run fixtures pass | Keep GitHub/raw scripts canonical; do not change default URL | Planned |
| LT033 | Read-only dashboard closure | Loopback/auth/redaction/no-write-route evidence | LT024 | HTTP tests pass with local socket access | Record sandbox PermissionDenied and rerun in approved local environment | Planned |
| LT034 | I118 closeout | Bounded productization candidate and owners synchronized | LT030-LT033 | full preflight, governance, docs | No release candidate if any boundary is unproven | Planned |
