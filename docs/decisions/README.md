...
| [076: Language Provider Default Distribution Policy](076-language-provider-default-distribution-policy.md) | **Proposed / LANG-003** | Preserves current defaults and requires explicit verified Bundle distribution, per-language fallback and measured compatibility evidence. No implementation authorization until accepted and claimed. |
| [077: Consented On-Demand Bundle Resolution](077-consented-on-demand-bundle-resolution.md) | **Proposed / DIST-001-B** | Requires exact-identity consent, verified bounded download/quarantine, separate activation and fail-closed network behavior. No implementation authorization until accepted and claimed. |
| [078: Read-Only Browser Provider Connector](078-read-only-browser-provider-connector.md) | **Proposed / BROWSER-001** | Constrains the first connector to bounded read-only `fetch_url` context, redaction, origin/TTL and permission/disclosure boundaries. No implementation authorization until accepted and claimed. |
