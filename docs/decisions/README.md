...
|---|---|---|
| [079: Rust Language Plugin Guest Buffer Boundary](079-rust-language-plugin-guest-buffer-boundary.md) | **Proposed / I278** | Additive guest-owned request buffer handshake; guest-only unsafe export attributes, no unsafe memory operations; independent security/API review and effective claim required before implementation. |
...
| [077: Consented On-Demand Bundle Resolution](077-consented-on-demand-bundle-resolution.md) | **Accepted / DIST-001-B** | Requires exact-identity consent, verified bounded download/quarantine, separate activation and fail-closed network behavior. |
| [078: Read-Only Browser Provider Connector](078-read-only-browser-provider-connector.md) | **Accepted / BROWSER-001** | Constrains the first connector to bounded read-only `fetch_url` context, redaction, origin/TTL and permission/disclosure boundaries. |
...
| [066: First-Class Scoped Permission Grants](066-first-class-scoped-permission-grants.md) | **Accepted / PERM-006-B Review/Claimed** | Decision content commit `17088d88` separates session grants from policy rules and freezes Deny/multi-facet/provenance/external-path boundaries plus the v0.9+ API migration. I219's claim became effective through PR #359 merge `781bb112`; its locally converged implementation still requires stable-candidate CI/review/CAS. |
| [067: Agent-Owned Permission Pipeline And Migration Contract](067-agent-owned-permission-pipeline.md) | **Accepted / I220 Complete** | Decision commits `c21bb7f3`/`820586ea`; PR #373 merged as `5d2d2dcf` after exact-head CI `32619757871`, independent permission/security/API review `5384374028` and CAS. I221 separately owns implementation. |
...
| [069: Model-First Permission Triage](069-model-first-permission-triage.md) | **Accepted / I242 Complete** | Decision-only boundary for deterministic-first, model-assisted triage of bounded read-only/local-validation requests. Accepted with decision content `8c570f84`, PR #457 merge `3e423259`, exact-head CI `33464911927` and independent reviews `5488254817` + `5488336387`; I241 implementation remains separately claimed and reviewed. |
...
| [059: Desktop Renderer, Host, And Motion Quality Boundary](059-desktop-renderer-host-motion-boundary.md) | **Accepted / I194 Complete / I277 Active** | Maintainer accepted isolated GPUI renderer/native/execution-domain boundary; dependency verification, actual platform behavior and measured interaction performance are not self-certified by acceptance. |
