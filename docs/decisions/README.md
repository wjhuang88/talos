...
|---|---|---|
| [065: Encapsulated Permission Rules And Diagnostic Provenance](065-structured-permission-rule-provenance.md) | **Accepted when reviewed status reaches `main` / I189 Active** | Decision content commit `dae98460` encapsulates `PermissionEngine` for truthful configured/runtime/default provenance. Serialized `PermissionRule` and decisions remain compatible, mode remains diagnostic, hook transport is deferred, and any future publication containing the source break must be v0.9.0 or later. Exact-head security/API review, CI and CAS still gate effectiveness. |
