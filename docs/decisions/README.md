...
35. [035: TUI Conversation History Scrollback Boundary](035-tui-history-scrollback-boundary.md) — Accepted (2026-07-03). Generalizes ADR-019 to conversation history: terminal scrollback stays the canonical renderer for finalized history; a viewport-rendered history view is rejected for the current direction. Documents the accepted costs (no re-wrap on resize, no retro theme change, no interactive history) against the benefits (native scroll/search/copy, zero retained render state, O(1) memory), with reversal only on a committed interactive-history requirement.
...
- [053: Explicit Tool Registration Contributions And Product Composition](053-tool-registration-composition.md) — **Proposed** (2026-07-26). Defines explicit contribution ownership (tool-owning crates contribute factories/descriptors), outer product composition (CLI/runtime select mode/capability and apply permission wrappers), and deterministic duplicate-name/source diagnostics; no global/implicit registration. Required before ARCH-034-R01 implementation (I158). Not yet Accepted; architecture/maintainer review pending.

- [054: Alternate-Screen Application-Owned Transcript Rendering](054-alternate-screen-app-owned-transcript-rendering.md) — **Rejected** (2026-07-27). Implemented as an I156 trial, then rejected because hiding the logo, shell history, and native conversation scrollback violated the required interaction model.

- [055: Primary-Screen App-Owned Transcript With Native History Projection](055-primary-screen-app-owned-transcript-native-projection.md) — **Proposed** (2026-07-27). Keeps geometry-free application transcript ownership while projecting newly committed entries exactly once to primary-screen native scrollback with ordinary newlines; one renderer, no Alternate Screen, DECSTBM, or reverse index.
