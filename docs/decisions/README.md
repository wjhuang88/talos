...
|---|---|---|
| [066: First-Class Scoped Permission Grants](066-first-class-scoped-permission-grants.md) | **Proposed / PERM-006-B Unclaimed** | Separates session grants from policy rules, freezes Deny/multi-facet/provenance/external-path boundaries and records the v0.9+ public API migration. Acceptance requires exact-head independent permission/security/API review and grants no implementation authority. |
