...
|---|---|---|
| [065: Encapsulated Permission Rules And Diagnostic Provenance](065-structured-permission-rule-provenance.md) | **Proposed / I189 Active** | Truthful configured/runtime/default provenance requires encapsulating `PermissionEngine`; serialized `PermissionRule` and decisions remain compatible, mode remains diagnostic, hook transport is deferred, and any future publication containing the source break must be v0.9.0 or later. Independent exact-head security/API review gates acceptance and implementation. |
