...
|---|---|---|
| [062: Typed Provider Retry Progress Boundary](062-typed-provider-retry-progress.md) | **Accepted when PR #321 reaches `main`; Proposed while open** | Provider retry progress uses a defaulted additive request-local typed channel, existing non-exhaustive Agent/session events and a distinct reconnecting phase. Retry policy, persistence and dependencies remain unchanged; independent exact-head architecture review, CI and CAS gate target-branch acceptance. |
