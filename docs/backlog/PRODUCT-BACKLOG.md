...
| --- | --- | --- | --- |
| 1 | Conversation context continuity (MEM-002 → I024) | P0 — agent cannot sustain multi-turn conversation without history. Infrastructure exists (JSONL storage, Compactor) but is not wired. Blocks daily use. | `docs/backlog/active/MEM-002-conversation-context-continuity.md`; `docs/iterations/I024-conversation-context.md` |
...
| 7.5 | LLM-based compaction (layers 4-5) | Deferred from I024 — layers 1-3 handle most sessions; layers 4-5 need provider access through session actor. | `docs/backlog/active/MEM-003-llm-compaction.md` |
...
| TUI-006 | Code Block Rendering — Rounded Border & Syntax Highlighting | Planned (sub-slice A: P2, unblocked; sub-slice B: P3, blocked on tree-sitter ADR) | P2 | Replace visible Markdown fence markers with rounded Unicode border (`╭╮╰╯│`) + language tag label. Sub-slice A (border only) is independent and can ship first. Sub-slice B (syntax highlighting via tree-sitter) requires an ADR for the new dependency and may align with future tree-sitter tooling integration. | `docs/backlog/active/TUI-006-code-block-rendering.md`; `docs/proposals/tui-stream-markdown-rendering.md` §Future Work |
...
| MEM-002 | Conversation context continuity | **Active** (→ I024) | P0 | P0 — agent must sustain multi-turn conversation with current-session history. Partial implementation has landed; next work is evidence audit, resume/runtime verification, compaction proof, and I024 closeout. | `docs/backlog/active/MEM-002-conversation-context-continuity.md`; `docs/iterations/I024-conversation-context.md`; `docs/roadmap/TWO-WEEK-HANDOFF-PLAN.md`; `docs/proposals/session-context-contamination.md`; ADR-016 |
| MEM-003 | LLM-based compaction (layers 4-5) | Planned | P3 | Deferred from I024 Day 4 audit. Layers 1-3 wired and sufficient for typical sessions. Layers 4-5 require provider access through session actor architecture change. | `docs/backlog/active/MEM-003-llm-compaction.md` |
