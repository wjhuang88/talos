...
| 1 | Dashboard availability in Logo prefix | TUI-037 / I202 is in Review through implementation PR #230 under effective claim merge `d801c8d1`. Close Issue #104 before the mainline long task after exact-head CI, terminal matrix, independent security review and CAS. The slice adds one display-only token-free plain-text Logo row, removes the success Tip and token log, and preserves failure Tips. No OSC 8, Dashboard product page, auth/bind or persistence change. | `docs/backlog/active/TUI-037-dashboard-logo-link.md`; `docs/iterations/I202-tui037-dashboard-logo-link.md`; ADR-031; ADR-054; Issue #104; PR #230 |
