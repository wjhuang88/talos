...
| --- | --- | --- | --- | --- | --- |
| TUI-004 | TUI state model — unified messages, tips, and event-bus hook | **Complete** (→ I023) | P1 | Implemented: event-driven `talos-conversation` boundary, Codex-style single-row history insertion, 3-column prefixes, multiline user blocks, one-row preview, Markdown block classifier, and conservative styled Markdown rendering for assistant/tool/system/error streams. Focused slice has 113 tests passing (60 TUI + 53 conversation). Remaining work is I023 closure: cancellation path, non-lossy state-critical delivery, engine-owned mutation, runtime verification, and closeout docs. Depends on I022 (Complete). | `docs/backlog/active/TUI-004-state-model.md`; `docs/iterations/I023-tui-state-model.md`; `crates/talos-tui/src/state.rs`; `crates/talos-tui/src/app.rs` |
