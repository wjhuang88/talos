...
| TUI-002 | TUI Codex-style modular overhaul | Planned | P2 | Structural refactor of `talos-tui` to mirror Codex's 80+ module layout (history_cell/, keymap.rs, bottom_pane/, slash_command.rs, tui/event_stream.rs, tui/frame_requester.rs). Blocked behind I015-I017 (R6-R8) so the new cells render the new provider/tool/git output formats from day one. Natural iteration slot: I022 or later. | `docs/backlog/active/TUI-002-codex-overhaul.md`; `docs/proposals/tui-codex-overhaul.md`; `docs/reference/REFERENCE-PROJECTS.md` §687-741; ADR-003; ADR-005; ADR-006 |
| TUI-003 | TUI exit transcript into terminal scrollback | Planned | P1 | TUI currently uses `EnterAlternateScreen`/`LeaveAlternateScreen` (`crates/talos-tui/src/app.rs:58-60, 614-625`); the alt-screen content is discarded on exit, so the conversation vanishes from the user's terminal. Codex TUI appends the transcript to the host scrollback on exit. Fix is a TUI lifecycle change in `Tui::Drop` that dumps `TuiState::transcript_plain_text()` to stdout after `LeaveAlternateScreen`. **No blockers**: lifecycle layer, not structural. Can land as the next iteration (I015-equivalent) ahead of TUI-002. | `docs/backlog/active/TUI-003-tui-exit-transcript.md`; `docs/iterations/I014-tui-completion.md`; `crates/talos-tui/src/app.rs:50-71, 614-625`; `crates/talos-tui/src/state.rs:477-503`; `docs/reference/REFERENCE-PROJECTS.md` §687-741 |
