...
| 1 | Dashboard availability in Logo prefix | TUI-037 / I202 is In Progress through effective claim merge `d801c8d1`. Implement and close Issue #104 before the mainline long task: one display-only token-free plain-text Logo row, no successful generic Tip, ADR-031-compliant token-free logs, failure Tip preserved. No OSC 8, Dashboard product page, auth/bind or persistence change. | `docs/backlog/active/TUI-037-dashboard-logo-link.md`; `docs/iterations/I202-tui037-dashboard-logo-link.md`; ADR-031; ADR-054; Issue #104 |
