...
| 0 | Change-aware CI routing | I190/GOV-005 proposes a fail-closed documentation-only classifier so prose changes retain governance/document gates without unconditional Unix/Windows Rust workspace tests. No CI authority exists before claim merge. | `docs/backlog/active/GOV-005-change-aware-ci-routing.md`; `docs/iterations/I190-change-aware-ci-routing.md` |
| 0 | Permission pipeline convergence | PR #197 proposes I189/PERM-006-A as the behavior-preserving structured request/context/report foundation; its claim has no authority before independent security review, exact-head validation and target-branch merge. B-E remain blocked in order, and PERM-007/#188 cannot bypass PERM-004/PERM-005 safety. | `docs/backlog/active/PERM-006-permission-pipeline-convergence.md`; `docs/backlog/active/PERM-006-A-structured-permission-decisions.md`; `docs/iterations/I189-perm006a-structured-permission-decisions.md`; PERM-006-B/C/D/E; `docs/backlog/active/PERM-007-model-assisted-goal-permission-decisions.md` |
...
| 1 | Runtime session and protocol foundations | I187/SESSION-008-A is in Review with Proposed ADR-058 and a current-path characterization under claim merge `5bb83f80`. Exact-head review/acceptance gates SESSION-008-B; that implementation must then close the durable partial-turn gap before RUNTIME-005-A/B/C. SESSION-009 remains Refinement; ACP-001 remains Blocked. | `docs/backlog/active/SESSION-008-interrupted-turn-partial-persistence.md`; `docs/iterations/I187-session008a-partial-turn-lifecycle.md`; `docs/decisions/058-partial-turn-durable-finalization.md`; `docs/backlog/active/SESSION-009-multi-client-session-architecture.md`; `docs/backlog/active/RUNTIME-005-bounded-graceful-shutdown.md`; `docs/backlog/active/ACP-001-agent-client-protocol-server.md` |
...
| ARCH-034-R04-AG12 / I185 | Review — implementation pending review | [AG-12](active/ARCH-034-R04-AG12-sqlite-validator-integrity.md) / [I185](../iterations/I185-sqlite-validator-policy-integrity.md) implement one structured ADR allowlist, unchanged `--locked` fail-closed policy and localized stderr preservation. No consumer or runtime change is included. |
