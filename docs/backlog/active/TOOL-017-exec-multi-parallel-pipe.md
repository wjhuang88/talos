...
## 2026-07-05 M1 Implementation

`exec` now accepts either the original single-command shape or a `steps` array. Step mode is
strictly sequential and stops after the first failed step; later steps are not spawned. Each step is
still executed through direct `tokio::process::Command` argv arguments, with no shell parsing,
glob expansion, redirection, background jobs, or pipe semantics.

The permission profile now exposes an `Execute` command facet for every step plus each step's
explicit cwd `Read` facet. Single-command usage remains backward compatible. Mixed
`command`+`steps` input is rejected as ambiguous, and sensitive environment names are denied for
both top-level single-command input and per-step input before spawning.

M2 parallel execution, M3 pipe chains, and M4 broader permission-rule semantics remain planned.
They should not be inferred from the M1 schema or output.
