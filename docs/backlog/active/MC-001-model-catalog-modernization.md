...
- [x] catalog.db has an explicit versioned migration path and corrupt/incompatible DB fallback.
      (schema v1 + `IncompatibleSchema` error; missing DB is created and seeded from packaged
      `models.toml`; corrupt/incompatible DB degrades to `builtin_models()` without overwrite)
...
- [x] Catalog-aware resolver reads from catalog.db when available, falls back to `builtin_models()`,
      then applies user config overrides. Fresh installs implicitly create a seeded catalog on first
      access. (`Config::all_models_with_catalog`/`resolve_model_limits_with_catalog`, S1-E)
...
- [x] `/connect` shows full provider list with credential entry and optional custom endpoint
      ...
      `catalog.db` is unseeded/unavailable/corrupt)
