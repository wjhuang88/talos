...
| Arborium/tree-sitter | Four parser construction/language/parse paths in `talos-tools/src/symbol.rs`; `talos-tui/src/highlight.rs` | Tool language/parse absence degrades to `None` or `ToolResult::error`; TUI highlighting catches panics and falls back after a completed call reports more than 500ms elapsed | Tool parser work runs synchronously inside async execution with no visible `catch_unwind` or deadline. The TUI elapsed check cannot interrupt a hung/long native call, so ADR-020's timeout requirement still needs review. Recursive tool workspace traversal is unbounded by file count/size/deadline. | Both crates in the reverse dependency tree; malformed/adversarial corpus, panic injection or adapter seam, enforceable deadline fixture, and proof of plain-text/error fallback without process exit. |
