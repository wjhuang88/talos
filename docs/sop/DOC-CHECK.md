...
- [ ] `.agent-governance/manifest.yaml` `status`, `last_audited_at`, and `next_actions` are current.
...
Run the governance validator before trusting the doc state:

```bash
python3 <skill-path>/scripts/validate_project_governance.py .
```
