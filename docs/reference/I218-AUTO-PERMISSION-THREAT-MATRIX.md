...
| Ineligible mutation | Modify, delete, rename, chmod, binary write, credential/config mutation, user-owned dirty-file overwrite or operation on `main` is classified low risk. | Initial allowlist admits only declared creation of a new structured text file under a typed managed-workspace lease on a non-`main` worktree. The request binds `TargetState::Absent`, parent identity and normalized path; the mutation point must use atomic no-clobber creation. | Table-driven operation-subtype, branch/worktree, lease, existing-target and dirty-resource fixtures. | Human; headless Deny. |
...
| Ambiguous model answer | Low confidence, contradictory reason, unknown enum, extra fields or invalid schema is accepted. | Closed output fields are `schema_version`, `request_digest`, `decision`, `reason_code` and `confidence`. `AllowOnce` requires `confidence=high`, reason `bounded_workspace_text_create`, known policy/model version and every deterministic validator predicate. | Property/fuzz tests for malformed/extra/conflicting fields. | Human; headless Deny. |
...
| Race/TOCTOU | Worktree, policy, mode, parent identity or target existence changes between assessment and execution. | The one-shot request/authorization binds `TargetState::Absent`, normalized path and a trusted open parent-directory capability/identity held through mutation. Creation is relative to that capability with atomic no-clobber semantics and fails if any target appeared. Path-string recheck plus `create_new` is insufficient for parent-swap defense; unsupported platforms remain ineligible. | Same-path parallel create, target-appears-after-assessment, parent/symlink/reparse-point swap, policy/mode generation and replay fixtures. | Deny without modifying the appearing target or writing outside the held parent capability. |
...
2. every facet is a workspace-local `Write` for creation of one new structured text file;
...
5. the request binds an absent target, normalized path and trusted open parent-directory
   capability/identity held through mutation; normalized resources are inside that workspace and
   exclude credentials, secret stores, executable metadata, VCS internals, modification, deletion,
   rename, chmod, binary data and sandbox fallback;
...
it supports bounded unattended creation without turning existing user content, general
command/network access or arbitrary user workspaces into model-owned authority. Automatic
modification needs a later reviewed atomic compare-and-write contract.
