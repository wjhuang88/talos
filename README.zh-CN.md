...
如果你在模型仍在处理时连续输入多条消息，它们会自动排队。当前 turn 完成后，Talos 会把当时队列中的所有输入按 FIFO 顺序合并为一个后续用户 turn，每条输入保持原文并以空行分隔；在该后续 turn 期间新到达的输入会进入下一批。TUI 在输入区上方显示排队消息的紧凑预览（最多 6 行；较长队列显示 `+N more` 摘要），批次取出后预览自动清空。
