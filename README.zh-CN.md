...
Talos 已发布第一条稳定的 pre-1.0 release 线。当前工作区版本是 `v0.10.1`。它已经可以用于
...
- **可配置的提供商与模型**：通过无参数的 `/connect` 和 `/model` 选择器添加 OpenAI 或 Anthropic 兼容提供商、发现模型并切换当前会话，不再解析命令字符串。
- **显式视觉附件**：仅能通过 `/attach`（或 print 模式的 `--attach`）附加 PNG、JPEG、GIF 或 WebP；会先完成能力、权限、格式、大小、像素和替换检查，普通文本中的图片路径绝不会被自动读取。
...
- **可扩展表面**：MCP 工具、Hooks、JSON-RPC、工程治理状态和显式本地只读 WASM 包已实现；远程插件分发与浏览器控制面仍单独受限。

## 当前 Release 边界

`v0.10.1` 适合本地开发者在自己机器上使用，并由操作者审查工具动作和配置。它还不是远程多用户服务、
...
- TUI 模式下默认启动只读 loopback dashboard；成功后会在仅用于显示的 TUI Logo 区域给出
  ...
  HTML 时继续保持现有 JSON/纯文本接口。`/activity` 新增基于有界 SSE 的 GET/只读实时工作区，
  展示当前 Session/模型/Turn、白名单语义活动、权威用量，以及成功写入并再次脱敏的运行日志；
  prompt/消息正文、thinking/reasoning、审批参数、原始工具输入/结果、凭据和未知事件变体不会
  进入该投影。重连仅使用进程内事件 ID 和有界 replay/reset，不构成持久 Session 历史。当前
  Dashboard UI 仅提供英文界面，双语
  ...
  TUI 不发送终端超链接转义序列。默认 loopback 模式是受支持的浏览器实时查看路径；要求认证
  的模式仍只支持显式请求头，不新增浏览器 token 交付机制。
- 本地模型配置，密钥显示自动脱敏。
...
- 通过可重复的 `--plugin DIRECTORY` 显式加载本地只读 WASM 包；未指定时不会加载，
  已加载工具仍经过权限、来源标记和资源上限。
...
- 插件市场、远程安装、自动发现、host call 和可写插件。
...
输入区会按终端宽度自动折行。`Shift+Enter` 插入换行，普通 `Enter` 提交。Talos 会先探测
终端是否支持增强键盘协议，再启用完整的修饰键上报；对于不支持该协议的终端或复用器，
可使用兼容性换行键 `Ctrl+J`。

在活动 turn 中按 `Esc` 可请求中断。`Ctrl+C` 清空输入区内容；输入区为空且空闲时，
连续按两次 `Ctrl+C` 退出 Talos。正常退出会恢复终端输入和显示模式；如果 Talos 被强制终止后
...
| `/model` | 打开模型选择器，在运行时浏览和切换模型；模型按提供商分组，当前模型显示在顶部分组，输入内容可按分组过滤。仅接受无参数形式：`/model gpt-4o` 会显示纠正信息并打开选择器 |
| `/connect` | 打开提供商选择器以连接新的提供商（凭据及可选的自定义端点/`base_url`）。仅接受无参数形式：`/connect openai` 会显示纠正信息并打开选择器 |

查看打包模型目录时，`--available-models` 默认只输出有上限的结果，模型行使用
`provider/model` 以避免重复模型名歧义：
...
使用 `/model` 在已配置凭据的提供商之间切换模型。选择器仅显示当前可用的模型，按提供商分组，并将当前使用的模型固定在"当前"分组中。`/model` 和 `/connect` 均为**严格无参数命令**：输入 `/model gpt-4o` 或 `/connect openai` 不会直接切换模型或开始凭据录入，而是显示一条纠正信息并打开相应选择器，让用户在面板中搜索和选择。使用 `/connect` 添加或更新提供商凭据。`/connect` 会从打包的离线 `models.toml` 显示提供商配置选项，提示输入 API key，并可选地提供自定义端点（`base_url`）以适配兼容网关的提供商。对于目录元数据已内置默认端点的标准提供商，在输入 API key 后可直接提交，无需填写 URL；自定义提供商（或任何没有内置端点的行）仍需填写非空的 `base_url`。`/connect` 选择器还提供**添加自定义提供商**入口，打开一个可取消的向导：输入提供商名称（1–64 位 slug）、选择协议（`openai-chat` 或 `anthropic-messages`）、输入 HTTPS base URL（仅 loopback 可用 HTTP）、输入 API key（已脱敏），然后确认。向导以一次原子配置更新保存所有字段——取消或任何验证错误都会保持现有配置不变。全新安装无需手动初始化目录：Talos 不再为模型元数据创建运行时 `catalog.db`。模型/提供商元数据更新只通过 `BUILD_MODELS=1` 在构建期完成；旧的 `--import-models` 标志仅保留为 no-op 兼容提示。
...
- Shell 兜底：Unix 使用 `bash`（通过 `sh -c`），Windows 使用 `powershell`（通过 `powershell.exe -NoProfile -NonInteractive`）；两者都经过执行权限管线

默认提示词会要求模型优先使用内置工具，只有在原生工具无法覆盖任务时才使用 Shell 命令兜底。
它也强调准确性优先于迎合：不能奉承、编造引用，证据不足时不能隐藏不确定性。

在标准 CLI/TUI/inline 组合中，`read` 会向当前模型提供紧凑的两位十六进制行锚点，后端以
有界内存文件快照和完整 revision 负责校验；`edit` 可以据此执行防止陈旧读取的原子行编辑，
同时继续兼容原有字符串替换输入。snapshot handle 和行 hash 只用于当前模型协作，不进入 Hook、
审批展示、可见历史、导出、Transcript 或 TLOG；展示和历史仍保留脱敏后的普通文件内容。
Runtime 重建或恢复后必须重新读取文件，所有写入仍按当前权限策略重新判断。

文件工具默认仍限制在工作区内。交互式权限组合访问工作区外路径时，Talos 会让用户选择；
批准只绑定当前工具操作和规范化后的准确路径。拒绝、headless 模式缺少批准处理器、跨路径或
跨操作复用、以及批准后符号链接目标变化都会安全拒绝。

TUI 中的批准结果会保留对应工具名；临时 `Calling tools...` 兼容标记会一直保留到真实结构化工具调用确认后才抑制。没有匹配调用的直接结果或审批事件会保留标记，不会生成错误的关联结果。
...
多个包可重复传入 `--plugin`；`/plugins` 只显示成功加载的包和已注册能力。未指定时原有
行为不变。
...
0.10.1 新增单一直接 Talos 依赖的 SDK：自定义 provider、工具、权限规则、sandbox
适配器与类型化事件所需的规范类型可从 `talos_runtime` 导入。这些新增路径首次随
`0.10.1` 发布。源码 `quickstart` 仅通过 facade 实现本地 provider；其他示例可以选用
...
Shell 权限申请可先由模型分析；显式配置的 `Ask` 仍保留人工确认，模型不能覆盖。
审核使用真实执行参数与工具提供的目录，可纳入工作区内脚本的完整有界快照；缺失、
超限、敏感或无法解析的依赖会明确标记。可变脚本快照不等于执行绑定，不能据此自动
放行。权限面板会展示可用的模型影响摘要与待确认事项，并区分审核失败和风险结论。
...
`v0.10.1` release 公告和下载以 GitHub Releases 为准。
