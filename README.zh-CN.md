...
Talos 已发布第一条稳定的 pre-1.0 release 线。当前工作区版本是 `v0.2.0`。它已经可以用于
...
`v0.2.0` 适合本地开发者在自己机器上使用，并由操作者审查工具动作和配置。它还不是远程多用户服务、
...
- 网络：`fetch_url`（有边界的 URL 上下文读取）、`http_request`（按需披露的高级 HTTP/API 检查）、`web_search`
- Shell 兜底：`bash`
