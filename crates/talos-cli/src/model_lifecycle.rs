...
/// Materializes the selected model variant through the shared configuration resolver.
pub(crate) fn materialize_runtime_model_config(
    config: &Config,
) -> (Config, talos_config::variant::VariantResolution) {
    talos_config::variant::materialize_runtime_model_config(config)
}
#[cfg(test)]
use talos_config::variant::resolve_variant;
