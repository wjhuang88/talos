...
fn max_tokens_is_visible_and_durable_as_truncated() {
    ...
    let session = manager.create_session("max-tokens", "").unwrap();
    ...
    session.append_terminal_diagnostic(&diagnostic).unwrap();
    let reopened = session.read_terminal_diagnostics().unwrap();
...
fn interactive_tlog_terminal_diagnostic_round_trips_with_turn_correlation() {
    ...
    session.append_terminal_diagnostic(&diagnostic).unwrap();
    let reopened = manager.get_session(&session.id).unwrap();
    let diagnostics = reopened.read_terminal_diagnostics().unwrap();
...
fn terminal_diagnostic_is_excluded_from_messages_copy_export_and_provider_history() {
    let temp_dir = tempfile::tempdir().unwrap();
    let manager = SessionManager::with_dir(temp_dir.path().to_path_buf());
    let session = manager.create_session("exclusion", "").unwrap();
    session
        .append(&Message::User {
    ...
    let provider_history = session.read_messages().unwrap();
    ...
    let transcript = read_transcript(&CompactTextSessionStore, &session.file_path).unwrap();
...
fn recent_terminal_diagnostic_survives_compaction() {
    let temp_dir = tempfile::tempdir().unwrap();
    let manager = SessionManager::with_dir(temp_dir.path().to_path_buf());
    let session = manager.create_session("compaction", "").unwrap();
    for index in 0..8 {
        session
            .append(&Message::User {
    ...
    let result = session.compact_archived(3).unwrap();
...
fn terminal_diagnostic_redacts_and_bounds_untrusted_fields() {
    ...
    assert!(diagnostic.reason.as_deref().unwrap().chars().count() <= 160);
    assert!(!diagnostic.reason.as_deref().unwrap().contains('\n'));
    assert!(!diagnostic.provider.as_deref().unwrap().contains('\n'));
    assert!(!diagnostic.model.as_deref().unwrap().contains('\r'));
}
